## FIXME: This is for now just the nloptr tests copied, so I have to decide
##        if I should rename it to "test_NLOPTR" or make it more general or both.
...
test_nloptr02 <- function(solver) {
    ...
    check("NLP-02@01", equal(res0$solution, solution.opt))
    ...
    ## Solve Rosenbrock Banana function.
    res1 <- ROI_solve( x, solver="nloptr", control )
    check("NLP-02@01", equal(res1$solution, solution.opt))
}
...
test_nloptr04 <- function(solver) {   
    ...
    # Run some checks on the optimal solution.
    check("NLP-04@01", equal(res$solution, solution.opt, tol = 1e-6 ))
