...
\newenvironment{sframe}{%
 \def\at@end@of@sframe{}%
 \ifinner\ifhmode%
  \def\at@end@of@sframe{\end{minipage}}%
...
\newenvironment{sweaverout}{}{} % an empty environment to be redefined in TeX
...
\begin{sweaverout}
\definecolor{shadecolor}{rgb}{0.957, 0.953, 0.937}\color{fgcolor}\begin{sframe}
\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in SOLVE(x, cntrl): CPLEX will not run unless you provide a valid email address.}}\end{sframe}
\end{sweaverout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in CVXR::psolve(a, b, ...): Problem does not follow DCP rules.}}\end{sframe}
\end{sweaverout}
