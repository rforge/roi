    ...
    if( any(row_valid) ){
        ...
        if( any(idx_cast) ){
            ...
            if( any(idx_cast) ){
                ...
                x$L$v[ x$L$i == which(row_valid)[idx_cast] ] <- 1
                eps <- c(">" = 1, "<" = -1)[x$dir[row_valid][idx_cast]]
                x$rhs[row_valid][idx_cast] <- x$rhs[row_valid][idx_cast]/coefs + eps
                x$dir[row_valid][idx_cast] <- sense_db[ x$dir[row_valid][idx_cast] ]
            } else {
                warning( "some constraints cannot be casted to non-strict inequalities" )
            }
...
## initialize ROI optimization problem
op <- OP( objective = c(3, 11, 6),
          constraints = L_constraint(L = matrix(c(-1, 1, 0, -2, 4, 0,
                                                  1, -1/4, 1/2), nrow = 3),
                                     dir = c("<=", "<=", "<"),
                                     rhs = c(4, 2, 3)),
          types = c("I", "C", "I"),
